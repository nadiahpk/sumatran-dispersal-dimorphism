...
# plot on top the isocline obtained by cyclical method
# ---

suffix_iso = '_1'
ID_iso = 0 # default parameter values I am also using here for the stochastic run

fname = '../../results/circular/isocline' + suffix_iso + '_' + str(ID_iso) + '.csv'
df = pd.read_csv(fname)
...
plt.plot(m_iso1v, m_iso2v, lw=3, color='black')
...
fname_in = '../../results/circular/sing_strat' + suffix_iso + '.csv'
df = pd.read_csv(fname_in)
ss_res = df.iloc[ID_iso]                # the particular row we want
m_ss = ss_res['m_ss']               # singular strategy

plt.scatter([m_ss], [m_ss], color='black', s=100, zorder=9999)
...
# Create the figure
plt.legend(handles=legend_elements, loc='best', framealpha=1, fontsize='small')

# plt.show()

fname = dir_results + 'ambiguous_yellow.pdf'
plt.tight_layout()
plt.savefig(fname)
plt.close()
